...
from data_exploration.helper import fetch_table, prepare_data_for_maps, reshape_land_cover_data, aggregate_average_change
...
def mock_conn() -> sqlite3.Connection:
    ...
    cursor = conn.cursor()
    cursor.execute("CREATE TABLE test_table (ï»¿ObjectId INTEGER, ISO3 TEXT, Country TEXT, Climate_Influence TEXT, Unit TEXT, Indicator TEXT, F1961 REAL, F1962 REAL, F1963 REAL);")
    cursor.executemany("INSERT INTO test_table VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);", [
        (1, 'USA', 'United States', 'Influence1', 'Unit1', 'Indicator1', 0.1, 0.2, 0.3),
...
def mock_df() -> pd.DataFrame:
    ...
    data = {
        ...
        'Country': ['United States', 'Canada', 'Mexico', 'Turks and Caicos', 'United Arab Emirates'],
        'Climate_Influence': ['Influence1', 'Influence2', 'Influence3', 'Influence1', 'Influence2'],
        'F1961': [0.1, 0.4, 0.7, 0.1, 0.4],
        'F1962': [0.2, 0.5, 0.8, 0.2, 0.5],
        'F1963': [0.3, 0.6, 0.9, 0.3, 0.6],
        'Unit': ['Unit1', 'Unit1', 'Unit1', 'Unit1', 'Unit1'],
        'Indicator': ['Indicator1', 'Indicator1', 'Indicator1', 'Indicator1', 'Indicator1']
    }
...
def test_fetch_table(mock_conn):
    ...
    expected_columns = ['ISO3', 'Country', 'Climate_Influence', 'Unit', 'Indicator', 'F1961', 'F1962', 'F1963']
