...
def test_delete_columns(mock_df: pd.DataFrame):
    ...
    """
    df = delete_columns(
        mock_df, mock_data["Annual_Surface_Temperature_Change"]["columns_to_delete"]
    )
...
def test_load_df_to_sqlite(monkeypatch, tmp_path, mock_df):
    """Test the load_df_to_sqlite function with a mock DataFrame
        and temporary database path.
